    ...
    public class CartController : Controller
    {
        ...
        private void SaveOrder(OrderModel model, Payment payment, string userId)
        {
            ...
            foreach (var item in model.CartModel.CartItems)
            {
                ...
                //orderItems'a ekleme yaparken orderitems' a adres vermen gerekir. Bunu order classının ctor'unda yapıyoruz.
                order.OrderItems.Add(orderitem);
            }
            _orderService.Create(order);
        }

        private void ClearCart(string cardId)
        {
            _cartService.ClearCart(cardId);
        }

        private Payment PaymentProcess(OrderModel model)
        {
            IyzipayCore.Options options = new IyzipayCore.Options();
            options.ApiKey = "Buraya Iyzipay Sanbox Apı key eklenmelidir";
            options.SecretKey = "Buraya Iyzipay Sanbox Secret key eklenmelidir";
            ...
            foreach (var item in model.CartModel.CartItems)
            {
                ...
                basketItem.Price = item.Price.ToString().Split(",")[0]; //virgüleden ayır ilk elemanı al.Virgülden dolayı hata verebilir burası.
